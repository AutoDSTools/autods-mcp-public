...
| `AUTODS_APP_BASE_URL` | no (derived from `MCP_ENV`) | Web app a rendered product links to: `https://platform.autods.com` in prod, `https://v2-staging.autods.com` everywhere else. An override for a one-off host, not something a deploy has to set — the derived value is what keeps a staging deploy from handing out production links |
