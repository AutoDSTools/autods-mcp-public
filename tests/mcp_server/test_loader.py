...
def test_loads_vendored_products_manifest(bundled_manifest_dir: Path) -> None:
    ...
    # 5 AutoDSApi ops (products/stores/bulk_actions) + 6 ProductsResearch ops
...
def test_loads_products_research_manifest(bundled_manifest_dir: Path) -> None:
    """E2: the hand-authored ProductsResearch read endpoints are registered,
    route to the products_research upstream, and are all read-only."""
    registry = build_registry(bundled_manifest_dir)
    ...
    for operation_id in expected:
        ...
        assert op is not None, f"{operation_id} not registered"
        assert op.base_url_key == "products_research"
